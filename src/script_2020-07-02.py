...
def ascending():
    ...
    lgd_arc = lgd[14042: 14042+1422]
    ax.tick_params(labelsize=25, width=2.9)
    ax.set_title("2020-07-02", fontsize=24)
...
def descending():
    ...
    # the starting epoch of the short arc on 2020-07-02 is 35319 [2s sampling]
    plt.rcParams["font.sans-serif"] = ["Microsoft YaHei"]
    plt.rcParams["axes.unicode_minus"] = False
    lri_x = np.loadtxt("../input/2020-07-02/LRI1B_2020-07-02_Y_04.txt", dtype=np.longdouble, skiprows=129)
    pod_c = np.loadtxt("../output/ddk7_C_2020-07-02.txt", dtype=np.longdouble, skiprows=6)
    pod_d = np.loadtxt("../output/ddk7_D_2020-07-02.txt", dtype=np.longdouble, skiprows=6)
    gnv_c = np.loadtxt("../input/2020-07-02/GNV1B_2020-07-02_C_04.txt", skiprows=148, usecols=[3, 4, 5])[::2]
    gnv_d = np.loadtxt("../input/2020-07-02/GNV1B_2020-07-02_D_04.txt", skiprows=148, usecols=[3, 4, 5])[::2]
    lgd_out = np.loadtxt("../input/2020-07-02/lgd20200702", dtype=np.longdouble)
    latlon = np.loadtxt("../input/2020-07-02/coor_2020-07-02_descend.txt", dtype=np.longdouble)
    ...
    fig, ax = plt.subplots(figsize=(16, 8))  # 2020-07-02
    plt.plot(latlon[:, 0], lgd[35319: 35319+1416], label="self")
    plt.plot(latlon[:, 0], lgd_out[35319: 35319+1416, 0], label="released(open-access)")
    pod_c_arc = gnv_c[35319: 35319+1416]
    pod_d_arc = gnv_d[35319: 35319+1416]
    los_arc = los[35319: 35319+1416]
    lgd_arc = lgd[35319: 35319+1416]
    ax.tick_params(labelsize=25, width=2.9)
    ax.set_xlabel('Lat [deg]', fontsize=20)
    ax.yaxis.get_offset_text().set_fontsize(24)
    ax.legend(fontsize=15, loc='best', frameon=False)
